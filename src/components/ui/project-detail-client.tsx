...
import { ArrowLeft, Calendar, ExternalLink, Github, Star, Users } from "lucide-react"
...
interface ProjectDetailClientProps {
  project: Project
  serializedMdx: SerializedMDXContent["serialized"] | null
  contentError: string | null
}

/**
 * Client-side wrapper for project detail page with animations
 */
  ...
  return (
    <div className="container max-w-4xl mx-auto px-6 py-12">
      <motion.div variants={staggerContainer} initial="initial" animate="animate" className="space-y-12">
        ...
        {/* Project Image */}
        <motion.div variants={staggerItem} className="relative aspect-video rounded-lg overflow-hidden">
          <Image
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2 space-y-8">
            {/* MDX Content or Description */}
            <motion.section variants={staggerItem}>
              {contentError && (
                <div className="mb-6 p-4 bg-yellow-50 border-l-4 border-yellow-400 text-yellow-800 rounded">
                  <p className="text-sm">
                    <strong>Note:</strong> Detailed content unavailable ({contentError}). Showing basic project
                    information instead.
                  </p>
                </div>
              )}
              ...
              {serializedMdx ? (
                <MDXContent serializedContent={serializedMdx} />
              ) : (
                <>
                ...
                <div className="space-y-3 text-sm">
                  {project.metrics.impact && (
                    <div>
                      ...
                      <p className="text-muted-foreground mt-1">{project.metrics.impact}</p>
                    </div>
                  )}
                  {project.metrics.users && (
                    <div>
                      <span className="font-medium">Users:</span>
                      <span className="ml-2 text-muted-foreground">{project.metrics.users.toLocaleString()}</span>
                    </div>
                  )}
                </div>
              </motion.section>
