...
import type { BlogCategory } from "@/types/enums"

interface BlogClientPageProps {
  posts: ContentData[]
}

export default function BlogClientPage({ posts }: BlogClientPageProps): React.ReactElement {
  const [searchQuery, setSearchQuery] = useState("")
  const [selectedCategory, setSelectedCategory] = useState<BlogCategory | "all">("all")

  // Dynamically get categories from actual blog post data
  const categories = Array.from(
    new Set(posts.map(post => post.metadata.category).filter(Boolean))
  ).sort() as BlogCategory[] // Sort alphabetically for consistency
  ...
  const filteredPosts = posts.filter(post => {
    ...
    const matchesCategory = selectedCategory === "all" || post.metadata.category === selectedCategory

    return matchesSearch && matchesCategory
  })

  // Transform ContentData to match the expected blog post format
  const transformedPosts = filteredPosts.map(post => ({
    id: post.slug,
    slug: post.slug,
    title: post.metadata.title || "Untitled",
    excerpt: post.metadata.description || "",
    category: post.metadata.category || "technical",
